...
implicit none
   ...
   real(KINDR)                :: PercentageWormOuterMove
   ...
   complex(KINDC),allocatable :: raman_worm(:,:)
   ...
   ! as array dimensions. 
   integer, parameter :: NWormSectors=17
...
!   NBin=get_Integer_Parameter("NBin")
   ...
   PercentageWormOuterMove = get_Real_Parameter("PercentageWormOuterMove")
  ...
  if(PercentageWormInsert.gt.0) then
   ...
   if( get_integer_parameter("WormMeasRaman") /= 0) then
      allocate(raman_worm(-N3iwb:N3iwb,-N3iwb:N3iwb))
      raman_worm = cmplx(0, kind=KINDC)
   endif
   ...
   if (get_Integer_Parameter("WormZMeasG") /= 0) then
      ! Z-space measurement of G during worm calculation
      b_wormZmeasG = .true.
   else
      b_wormZmeasG = .false.
   end if
   ...
   Gtau_full=0d0
   ntau_n0=0d0
   if (allocated(GLeg)) GLeg=0d0
   if (allocated(GLeg_full)) GLeg_full=0d0
   Histo=0d0
...
!   if(allocated(LegendreGrid))deallocate(LegendreGrid)
   ...
   if(allocated(raman_worm)) deallocate(raman_worm)
...
subroutine measure_raman_worm()
   implicit none
   integer                    :: in1,in2,in3,k
   real(KINDR)                :: prefact
   integer                    :: s(6), o(6), bs(6), i

   
   prefact=1d0/wormEta(SectorRaman)
   ...
   !we only fourier transform new worm configs, old ones are just accumulated
   if(isNew(SectorRaman).eqv..true.) then
   
      !fourier transform if buffer is full
      if(tau_fill .eq. size(val_worm)) then

         allocate(matsubaras((4*N3iwb+3)*(4*N3iwb+3)),d(2))
         d(1)=4*N3iwb+3
         d(2)=4*N3iwb+3
         ...
         do in1=-N3iwb,N3iwb
            do in2=-N3iwb,N3iwb
               k=(4*N3iwb+3)*(2*(in2+N3iwb)+1)+(2*(in1+N3iwb)+1)+1 ! TODO check this!
               raman_worm(in1,in2)=raman_worm(in1,in2) + matsubaras(k)
            enddo
         enddo
      ...
      !raman operator comes in a three pairs in the order 2,1; 4,3; 6,5
      tau_worm(2*tau_fill+2) = (DTrace%wormContainer(6)%p%tau-DTrace%wormContainer(2)%p%tau)/(2d0*DTrace%beta)
      tau_worm(2*tau_fill+1) = (DTrace%wormContainer(6)%p%tau-DTrace%wormContainer(4)%p%tau)/(2d0*DTrace%beta)

      val_worm(tau_fill+1)=DTrace%cfgsign*prefact
      tau_fill = tau_fill + 1
      isNew(SectorRaman)=.false.
...
!local
   ...
   logical :: worm_outer, qncheckresult
   integer :: possible_ssts_old, possible_ssts
   integer :: ssts_old(1:size(DTrace%States(:, 1))), ssts_new(1:size(DTrace%States(:, 1)))
   ...
   if (grnd() < PercentageWormOuterMove) then
      worm_outer = .true.
   else
      worm_outer = .false.
   end if
   ...
   if (worm_outer) then
      call save_outer_sst(DTrace)
      possible_ssts_old = check_qn_all_ssts(DTrace, DStates, ssts_old)
   else
      ! determine substring of trace that needs quantum number checking
      begin%p => Oper(1)%p%prev
   ...
   if (worm_outer) then
      possible_ssts_old = check_qn_all_ssts(DTrace, DStates, ssts_old)
      qncheckresult = (possible_ssts <= 0)
   else
      call save_outer_sst(DTrace)
      qncheckresult = (.not. check_sst_sequence(DTrace, DStates, begin, endp, begin_sst, end_sst))
   end if

   
   !now we can check the quantum number violations
   if((.not.check_EqualTime(DTrace,Sector)).or.(prefact.eq.0d0)&
       .or. qncheckresult) then
   ...
   if (worm_outer) then
      if (DTrace%b_statesampling) then
         prefact = prefact * WeightedOuterStateChoice(DTrace, DStates,&
                              DTrace%beta - DTrace%last%tau + DTrace%first%tau,&
                              DTrace%beta - DTrace%last%tau + DTrace%first%tau,&
                              DTrace%outer_sst,&
                              DTrace%outer_state,&
         ...
         prefact = prefact * real(possible_ssts, KINDR)/real(possible_ssts_old, KINDR)
         iO = ceiling(grnd()*dble(possible_ssts))
         do i = 1, size(ssts_new)
            if (ssts_new(i) /= -1) then
               iO = iO - 1
               if (iO <= 0) then
                  DTrace%outer_sst = ssts_new(i)
                  exit
               end if
            end if
         end do
...
!local
   ...
   if (worm_outer) then
      if (DTrace%b_statesampling) then
         sst_factor = WeightedOuterStateChoice(DTrace, DStates,&
                              DTrace%beta - DTrace%last%tau + DTrace%first%tau,&
                              DTrace%beta - DTrace%last%tau + DTrace%first%tau,&
                              DTrace%outer_sst,&
                              DTrace%outer_state,&
         ...
         sst_factor = real(possible_ssts, KINDR)/real(possible_ssts_old, KINDR)
         iO = ceiling(grnd()*dble(possible_ssts))
         do i = 1, size(ssts_new)
            if (ssts_new(i) /= -1) then
               iO = iO - 1
   ...
   rand=grnd()
   if(rand.lt.dabs(sst_factor/prefact&
      *trval(TraceNew/DTrace%Trace)*BosTraceNew/DTrace%BosonicTrace))then
...
integer                           ::  bs(NOperWorm(Sector))
   ...
   elseif(Sector .eq. SectorUccaa) then
     ...
     prefact = -u_antisymm_1 & ! minus, because 2413 is odd permutation of 1234
       ...
       * DTrace%beta**2 &
       * Nbands**4
   elseif(Sector .eq. SectorRaman) then
     prefact = wormEta(Sector) * DTrace%beta**3 ! TODO necessary Nbands**6 factor?
   endif
...
subroutine count_steps(iSector, iComponent, Nwarmup, Nsteps, stepsWorm, stepsZ)
  use type_progress

  integer, intent(in)  :: iSector, iComponent, Nwarmup, Nsteps
  integer, intent(out) :: stepsWorm, stepsZ
...
!local
  ...
  integer                    :: bs(2), b(2), s(2)
   ...
   p%adaptrate = 0.1
   call pstart(p, int(Nsteps + Nwarmup, PINT), &
               title='EtaStep' // simstr // ':', fancy=fancy_prog)
   ...
   if(b_offdiag .and. iSector .eq. SectorG) then
      ...
      do j=1,(Nsteps + Nwarmup)
         if(any_signal_fired) then
            aborted = 1
            return
         endif
         if(Sector==SectorZ) then
            if (j > Nwarmup) CntSampling(SectorZ) = CntSampling(SectorZ) + 1
            rand_num=grnd()
         ...
         !elseif(Sector .eq. iSector)
            ...
            if (j > Nwarmup) CntSampling(iSector) = CntSampling(iSector) + 1
            rand_num=grnd()
            !insertion of hyb pair
...
!local
   ...
   if(iComponent.ne.0) then
      ...
      elseif(iSector .eq. SectorP3 &
        ...
        .or. iSector .eq. SectorQQdd &
        .or. iSector .eq. SectorRaman) then
...
!local
   ...
   real(KINDR)                :: rand_num
   real(KINDR)                :: time_start, time_temp, last_stopfile_check
   ...
   integer                    :: i_sample, max_tick, jobidlen
   character(len=:), allocatable           :: slurm_job_id

   call get_environment_variable("SLURM_JOB_ID", length=jobidlen)
   if (jobidlen > 0) then
      allocate(character(len=jobidlen) :: slurm_job_id)
      call get_environment_variable("SLURM_JOB_ID", value=slurm_job_id)
      last_stopfile_check = 0.0_KINDR
   else
      last_stopfile_check = huge(last_stopfile_check)
   end if
   ...
   do iNMeas=1,NMeas
      ...
      if (jobidlen > 0 .and. time_temp > last_stopfile_check + 30) then
         if (check_stopfile_present()) then
            aborted = 1
            return
         end if
         last_stopfile_check = time_temp
      end if
      ...
      if(Sector==SectorZ) then
         ...
         if (iSector == SectorZ .or. b_wormZmeasG) then
            if(b_offdiag)then
...
!           if(allocated(gsigmaiw)) call urho_contracted(u_matrix, dtrace, dstates)
       ...
       elseif(Sector==SectorRaman) then
            CntMeas(Sector) = CntMeas(Sector) + 1
            if(allocated(raman_worm)) call measure_raman_worm()
       endif 
   ...
   if(NMeas == 0) then
      ...
      enddo
      if (allocated(GLeg)) GLeg(:,:,:)=-GLeg(:,:,:) * zScaling
      if (allocated(GLeg_full)) GLeg_full(:,:,:,:,:)=-GLeg_full(:,:,:,:,:) * zScaling
      ...
      if(iSector==SectorRaman) wormNorm=1d0/(DTrace%beta)
      ...
      if(allocated(raman_worm)) then
        ...
        if(tau_fill .ne. 0) then
           ...
           do i=-N3iwb,N3iwb
              do j=-N3iwb,N3iwb
                  k=(4*N3iwb+3)*(2*(j+N3iwb)+1)+(2*(i+N3iwb)+1)+1 ! TODO check this is the same as above
                  raman_worm(i,j)=raman_worm(i,j)+matsubaras(k)
              enddo
           enddo
         ...
         raman_worm(:,:)=-raman_worm*wormScaling(SectorRaman)
...
contains
   logical function check_stopfile_present()
      inquire(file=(slurm_job_id // ".stop"), exist=check_stopfile_present)
   end function check_stopfile_present
