...
def lattice_el_result(gdensold, gdensold_err=None):
    """number of electrons in the lattice model, total and per spin"""
    # (..., orb, sp, orb, sp) -> (..., sp)
    gdensnew = np.diagonal(np.trace(np.real(gdensold), 0, -4, -2), 0, -2, -1)
    return (gdensnew[..., 0], gdensnew[..., 1], np.sum(gdensnew, axis=-1))
...
def lattice_dens_result(gdensold, gdensold_err=None):
    """number of electrons in the lattice model per band and spin"""
    # (..., orb, sp, orb, sp) -> (orb, sp)
    return np.diagonal(np.diagonal(np.real(gdensold), 0, -4, -2), 0, -3, -2)
...
derived_quantities = {
    ...
    "latt-dens-res": meta_from_func(lattice_dens_result,
                        ["lda-band", "spin"], ["value"]
                        ),
    "latt-el-res": meta_from_func(lattice_el_result,
                        [], ["spin-up", "spin-dn", "total"]
                        ),
