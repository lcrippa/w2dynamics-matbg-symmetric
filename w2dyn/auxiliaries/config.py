...
def doublecounting_from_cfg(cfg, ineq_list, mylattice, atom_list, u_full):
    ...
    elif dc_type == 'fixed_fll':
        if cfg["General"]["dc_fixed_dens"] is None:
            raise ValueError("dc = fixed_fll requires manually specified "
                             "densities per shell in dc_fixed_dens")
        return doublecounting.FixedFLL(
            np.array(cfg["General"]["dc_fixed_dens"], dtype=np.double),
            lda_dens, atom_list, u_full
        )
    elif dc_type == 'sc_fll':
        return doublecounting.SCFLL(ineq_list, lda_dens, atom_list, u_full)
    elif dc_type == 'Wterm':
