...
def weighted_combine(*samples, weights=None):
    """Combine a set of DistributedSample instances into one
    DistributedSample instance containing samples created by combining
    individual samples from the arguments in a weighted average using
    the standard errors of the arguments.
    """
    if len(samples) <= 0:
        raise ValueError("Must pass list of at least one sample")
    ...
    if weights is None:
        weights = np.array(tuple(np.nan_to_num(1.0/sample.stderr()**2,
                                               nan=1.0)
                                 for sample in samples))

    local = np.average(tuple(np.average(sample.local, axis=0)
                             for sample in samples),
                       axis=0,
                       weights=weights)[np.newaxis, ...]

    return DistributedSample(local, mpi_comm, mpi_comm.Get_size())
