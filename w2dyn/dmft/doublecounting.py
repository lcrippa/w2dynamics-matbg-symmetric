...
class FixedFLL(ShellAveraged):
    r"""Double counting using FLL formulas, but instead of DFT shell
    densities with user-specified ones (e.g. nominal valences).
    """
    def __init__(self, dens_sum, *args):
        ShellAveraged.__init__(self, *args)

        if not self.dens_sum.shape == dens_sum.shape:
            raise ValueError("Shape of dc_fixed_dens is {}, but should be {}"
                             .format(dens_sum.shape, self.dens_sum.shape))

        self.dens_sum = dens_sum
        self.dens_mean = [dsum / size / 2 for dsum, size in zip(dens_sum, self.sizes)]
...
class SCFLL(SelfConsistent):
    def get(self, siws=None, smoms=None, giws=None, occs=None):
        if occs is not None:
            self.densities = orbspin.promote_diagonal(
                np.reshape(np.real(self.shell_av['densities']), (-1, 2))
            )
            for ineq, occ_slice in zip(self.ineq_list, occs):
                ineq.d_setpart(self.densities, np.real(occ_slice))
            self.densities = np.reshape(orbspin.extract_diagonal(self.densities),
                                        (-1,))
        else:
            self.densities = self.shell_av['densities']

        dc_fll = FullyLocalisedLimit(self.densities,
...
class Wterm(SelfConsistent):
    def __init__(self, w, v, shifts, *args, **kwargs):
    ...
    def get(self, siws=None, smoms=None, giws=None, occs=None, densities=None):
        if densities is None:
            densities = self.shell_av['densities']
        densities = np.real(densities)

        shifts = np.array(self.shifts)
        shell_dc = []
        for atom in self.atom_list:
            nu_corr = np.sum(densities[atom.dslice, :]
                             - np.full_like(densities, 0.5)[atom.dslice, :])
            nu_uncorr = sum(np.sum(densities[sl, :]
                                   - np.full_like(densities, 0.5)[sl, :])
                            for sl in atom.ligslices)
            ...
            for i, shift in enumerate(atshifts):
                if i == 0:
                    shell_dc.append(self.w * (nu_uncorr - nu_corr) - self.v * nu_uncorr + shift)
                else:
                    #shell_dc.append(self.w * nu_corr + shift)
                    shell_dc.append(0.0 + shift)

        self.sav.from_shell(np.array(shell_dc))
        self.dc_value = -self.sav.dc_value
        return self.dc_value
