...
class DMFTStep:
    def __init__(self, beta, lattice, ineq_list, niwf, nftau, dc_dp, dc_dp_orbitals, GW, GW_KAverage, natoms, dc=None,
        ...
        if mixer is None: mixer = mixing.FlatMixingDecorator(mixing.LinearMixer())
    ...
    def set_siws(self, siw_dd=None, smom_dd=None, dc_full=None, init=False,
                 hartree_start=False, giws=None, occs=None, mu=None, muresid=None, mix=True):
        ...
        if mix:
            if muresid is not None:
                (self.dc_full, self.siw_dd, self.smom_dd), self.mu = (
                    self.mixer(self.dc_full,
                               siw_dd,
                               smom_dd,
                               custom_residual=(mu,
                                                muresid))
            ...
            # just store, e.g. to have the pure current self-energies
            # to update the mu value without influence from mixing or
            # because we are doing statistics iterations
            self.siw_dd, self.smom_dd, self.mu = siw_dd, smom_dd, mu
    ...
    def get_mu(self, target_totdens, epsn, search_range):
    ...
    def get_loc_imp_mixdens(self):
        ...
        return orbspin.extract_diagonal(mixdm)

    def get_old_mu_quantities(self):
        return (self.glociw, self.densmatrix)

    def write_gloc(self, output, infix, full_od=False, glociw=None, densmatrix=None):
        if glociw is None:
            glociw = self.glociw
        if densmatrix is None:
            densmatrix = self.densmatrix
        ...
        if self.mpi_comm is not None:
            output.write_distributed("gloc{infix}".format(infix=infix),
                                     self.mpi_strategy.parts(glociw),
                                     self.mpi_strategy.shape(glociw))
            glociw = self.mpi_strategy.allgather(glociw)
        else:
    ...
    def write_before_mu_search(self, output, full_od=False,
                               glociw=None, densmatrix=None):
        self.write_gloc(output, "old", full_od=full_od,
                        glociw=glociw, densmatrix=densmatrix)
...
class KappaMuExtrapolator:
    ...
    def initialize(self, all_mus, all_ns, all_nerrs=None):
        self.last_mu = all_mus.pop()
        self.last_n = all_ns.pop()
        mu = self.last_mu
        n = self.last_n
        ...
        if len(all_mus) > 0:
            ...
            kappa = (n - last_n) / (mu - last_mu)
            # try to find kappa with desired sign in earlier
            # iterations if necessary
            while (self.forced_kappa_sign is not None
                   and len(all_mus) > 0
                   and np.sign(kappa) != np.sign(self.forced_kappa_sign)):
                mu = last_mu
                n = last_n
                last_mu = all_mus.pop()
                last_n = all_ns.pop()
                kappa = (n - last_n) / (mu - last_mu)
            if not (((self.forced_kappa_sign is None
                      or np.sign(kappa) == np.sign(self.forced_kappa_sign))
                     and last_mu != mu and last_n != n)):
                kappa = None
                last_mudiff = None
...
class CurveFitter:
    def __init__(self, totdens, startat, minhistory, maxhistory, termminhist,
                 termdiffthresh, termdiffminlen, termntoterrfact,
                 termntotminlen, log=None):
        self.totdens = totdens
        self.count = 0
        ...
        self.nerrs = np.array([])
        self.coeffs = None
        self.termminhist = termminhist
        self.termdiffthresh = termdiffthresh
        self.termdiffminlen = termdiffminlen
        self.termntoterrfact = termntoterrfact
        self.termntotminlen = termntotminlen
        self.lockmu = None
        if log is not None:
            self.log = log
        else:
    ...
    def _refit(self):
        if self.mus.shape[0] < self.minhistory or self.lockmu is not None:
            return

        self.mus = self.mus[-self.maxhistory:]
        self.ns = self.ns[-self.maxhistory:]
        self.nerrs = self.nerrs[-self.maxhistory:]

        def polyfun(x, p0, p1):
            # x: polynomial coeffs
            return p1 * x + p0

        def jacobian(x, p0, p1):
            # x: second order polynomial coeffs
            jac = np.zeros((x.shape[0], 2), dtype=x.dtype)
            jac[:, 0] = 1.0
            jac[:, 1] = x
            return jac

        results = []
        for i in range(self.minhistory, self.mus.shape[0] + 1):
            results.append(opt.curve_fit(polyfun,
                                         self.mus[-i:],
                                         self.ns[-i:],
                                         p0=self.coeffs,
                                         sigma=(self.nerrs[-i:] if not np.any(np.isnan(self.nerrs[-i:])) else None),
                                         absolute_sigma=True,
                                         jac=jacobian))
            chisq = np.sum(((polyfun(self.mus[-i:], *results[-1][0]) - self.ns[-i:])/self.nerrs[-i:])**2)
            chicdf = stats.chi2.cdf(chisq, i - len(results[-1][0]))
            self.log("Fit history {}: chi^2 = {:.4f}, cdf = {:.4f}, 1 - cdf = {:.4f}".format(i, chisq, chicdf, 1 - chicdf))
        self.coeffs = results[-1][0]

        self.log("Fit result: n = {} * mu + ({})".format(self.coeffs[1], self.coeffs[0]))

    def _check_term_cond(self):
        if self.lockmu is not None:
            return
        if (len(self.mus) >= self.termminhist
                and np.all(np.abs(np.diff(self.mus)[-self.termdiffminlen:]) < self.termdiffthresh)
                and np.all(np.abs(self.totdens - self.ns[-self.termntotminlen:]) < self.termntoterrfact * self.nerrs[-self.termntotminlen:])):
            self.lockmu = np.mean(self.mus[-(self.termdiffminlen+1):])

    def initialize(self, mus, ns, nerrs=None):
        self.mus = np.append(self.mus, mus, axis=0)
        self.ns = np.append(self.ns, ns, axis=0)
        if nerrs is not None:
            self.nerrs = np.append(self.nerrs, nerrs, axis=0)
        else:
            self.nerrs = np.append(self.nerrs, [np.nan for x in ns], axis=0)
        self._check_term_cond()

    def has_mu(self):
        self._check_term_cond()
        return (self.lockmu is not None
                or ((self.mus.shape[0] >= self.minhistory)
                    and (self.count >= self.startat)))
    ...
    def next_mu(self):
        ...
        if self.lockmu is not None:
            return self.lockmu
        self._refit()
        next = (self.totdens - self.coeffs[0]) / self.coeffs[1]
        return (next
                if np.isfinite(next)
                else self.mus[-1])
    ...
    def step(self, mu, n, nerr=None):
        self.count += 1
        self.mus = np.append(self.mus, [mu], axis=0)
        self.ns = np.append(self.ns, [n], axis=0)
        if nerr is not None:
            self.nerrs = np.append(self.nerrs, [nerr], axis=0)
        else:
            self.nerrs = np.append(self.nerrs, [np.nan], axis=0)
