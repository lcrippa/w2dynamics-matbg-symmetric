...
def flush_all_python_output(mpi_comm=None):
    """
    Flush buffers of the standard output and standard error streams of
    the local process and optionally wait for other processes to do
    the same.

    :param mpi_comm: MPI communicator for barrier synchronization
    """
    sys.stdout.flush()
    sys.stderr.flush()
...
def exit_on_signal(ctqmc, mpi_comm=None):
    abortarray = np.array(ctqmc.aborted, dtype=np.int)
    mpi_comm.Allreduce(mpi.IN_PLACE, abortarray, op=mpi.SUM)
    if abortarray[...] > 0:
        print("CTQMC caught signal, exiting")
        sys.exit(0)
...
class ImpurityResult:
    ...
    def extend_by_bin(self, giw, gsigmaiw=None, smom=None, **other):
        self.giw = statistics.join(self.giw, giw)
        if gsigmaiw is not None:
            self.gsigmaiw = statistics.join(self.gsigmaiw, gsigmaiw)
        if smom is not None:
            self.smom = statistics.join(self.smom, smom)
        for key in other:
            self.other[key] = statistics.join(self.other[key], other[key])
        return self

    def postprocessing(self, siw_method="dyson", smom_method="estimate", ntau=0, muaux=None):
        ...
        def fourier_transform_siw(siw_in):
            from w2dyn.dmft.selfcons import iw_to_tau_fast
            smom = self.smom.mean()
            siw_in = -(siw_in - smom[0, np.newaxis, ...])/smom[1, np.newaxis, ...]
            siw_in = np.diagonal(np.diagonal(siw_in, axis1=1, axis2=3),
                                 axis1=1, axis2=2)
            iwf = tf.matfreq(self.problem.beta,
                             'fermi',
                             siw_in.shape[0])[:, np.newaxis, np.newaxis]
            stau = -iw_to_tau_fast(
                siw_in + 1.0j/iwf,
                ntau,
                self.problem.beta,
            ...
            return np.transpose(stau, (1, 2, 0))

        if ntau > 0:
            self.other["stau-ft"] = DistributedJackknife(self.siw).transform(fourier_transform_siw)

        def fourier_transform_saux(siw_in, muaux):
            from w2dyn.dmft.selfcons import iw_to_tau_fast
            smom = self.smom.mean()
            siw_in = (siw_in - smom[0, np.newaxis, ...])
            siw_in = np.diagonal(np.diagonal(siw_in, axis1=1, axis2=3),
                                 axis1=1, axis2=2)
            iwf = tf.matfreq(self.problem.beta,
                             'fermi',
                             siw_in.shape[0])[:, np.newaxis, np.newaxis]
            gaux = 1/(1.0j * iwf - siw_in + muaux)
            gtau = -iw_to_tau_fast(
                gaux + 1.0j/iwf,
                ntau,
                self.problem.beta,
            ...
            return np.transpose(gtau, (1, 2, 0))

        if muaux is not None:
            self.other["stau-aux0"] = (
                DistributedJackknife(self.siw)
                .transform(lambda siw: fourier_transform_saux(siw, 0.0))
            )
            self.other["stau-auxmu"] = (
                DistributedJackknife(self.siw)
                .transform(lambda siw: fourier_transform_saux(siw, muaux))
            )

    def calculate_siw(self, method, explicit_giw=None):
        ...
        giw_jk_input = DistributedJackknife(self.giw if explicit_giw is None else explicit_giw)
...
class CtHybSolver(ImpuritySolver):
    ...
    def set_problem(self, problem, compute_fourpoint=0):
        ...
        self.all_params = self.param_string + problem_params + symm_params
        self.fourpnt = compute_fourpoint
    ...
    def solve(self, iter_no, mc_config_inout=None):
        flush_all_python_output(self.mpi_comm)

        firstrun = True
        current_mc_config = None
        taudiffmax = None
        if isinstance(mc_config_inout, list) and len(mc_config_inout) > 0:
            firstrun = False
            current_mc_config = mc_config_inout.pop()

        total_result = None

        for ibin in range(self.config["QMC"]["NBin"]):
            # Call CT-QMC solver
            ctqmc.simid = self.mpi_rank
            time_qmc = time.clock()
            ctqmc.init_paras(self.all_params)
            ctqmc.fourpnt = self.fourpnt
            ctqmc.init_solver(self.umatrix, self.ftau, self.muimp, self.screening, iter_no+1)

            if current_mc_config is not None:
                current_mc_config.set_to_ctqmc(ctqmc)

            if self.config["QMC"]["segment"] == 0:
                if taudiffmax is None:
            ...
            exit_on_signal(ctqmc, self.mpi_comm)
            ...
            current_mc_config = CtHybConfig.get_from_ctqmc(ctqmc)
            firstrun = False
            ...
            if total_result is None:
                total_result = StatisticalImpurityResult(self.problem, giw, gsigmaiw, smom,
                                                         self.g_diagonal_only, **result)
            else:
                total_result.extend_by_bin(giw, gsigmaiw, smom, **result)
            result = None
        ...
        if isinstance(mc_config_inout, list):
            mc_config_inout.append(current_mc_config)

        def fourier_transform_giw(giw_in):
            from w2dyn.dmft.selfcons import iw_to_tau_fast
            giw_in = np.diagonal(np.diagonal(giw_in, axis1=1, axis2=3),
                                 axis1=1, axis2=2)
            iwf = tf.matfreq(self.problem.beta,
                             'fermi',
                             giw_in.shape[0])[:, np.newaxis, np.newaxis]
            gtau = -iw_to_tau_fast(
                giw_in + 1.0j/iwf,
        ...
        if self.config["QMC"]["FTGiwGtau"]:
            result["gtau-ft"] = DistributedJackknife(total_result.giw).transform(fourier_transform_giw)
    ...
    def solve_component(self,iter_no,isector,icomponent,mc_config_inout=[]):
        ...
        if self.config["QMC"]["Nwarmups2Plus"] < 0:
            if int(self.config["QMC"]["WormSearchEta"]) != 0:
                warn("Nwarmups2Plus not set; "
                     "assuming 0.1 * Nwarmups counted steps per eta iteration")
            EtaStepCounts = 0.1 * self.config["QMC"]["Nwarmups"]
        else:
            EtaStepCounts = self.config["QMC"]["Nwarmups2Plus"]
        WormWarmups = EtaStepCounts  # passed to ctqmc_worm_warmup, now unused

        if self.config["QMC"]["NwarmupsEta"] is None:
            EtaWarmupCounts = EtaStepCounts
            if int(self.config["QMC"]["WormSearchEta"]) != 0:
                warn("NwarmupsEta not set; "
                     "performing as many warmup steps as counted steps")
        else:
            EtaWarmupCounts = self.config["QMC"]["NwarmupsEta"]

        def eta_count_steps(eta, nwarmup, nsteps):
            """
            Count steps in worm and Z space for specified eta.

            :param float eta: weighting factor for worm space
            :param int nwarmup: number of warmup steps before starting to count
            :param int nsteps: total number of counted steps to perform

            :return: worm steps and relative difference of worm and Z steps
            :rtype: tuple[:class:`w2dyn.statistics.DistributedSample`, float]
            :raises AssertionError: if CTQMC returns inconsistent step counts
            """
            ctqmc.wormeta[isector - 2] = eta
            steps_worm, steps_z = ctqmc.count_steps(isector, icomponent,
                                                    nwarmup, nsteps)
            exit_on_signal(ctqmc, self.mpi_comm)
            assert steps_worm + steps_z == nsteps, \
                'QMC missed steps: {} in worm + {} in Z != requested {}' \
                .format(steps_worm, steps_z, nsteps)
            steps_worm = DistributedSample([float(steps_worm)], self.mpi_comm)
            reldiff = (2.0 * steps_worm.mean() - nsteps)/nsteps
            swmean = steps_worm.mean()
            szmean = nsteps - steps_worm.mean()
            if self.mpi_rank == 0:
                print('At eta={}: {} worm steps, {} Z steps -> {}'
                      .format(eta,
                              swmean,
                              szmean,
                              reldiff))
            flush_all_python_output(self.mpi_comm)
            return steps_worm, reldiff

        if int(self.config['QMC']['WormSearchEta']) == 1:
            def approx_equal(v1, err1, v2, err2=0.0):
                return np.abs(v1 - v2) <= err1 + err2

            # Patrik's eta search
            eta_stat = None
            eta = float(self.config['QMC']['WormEta'])
            max_iter = self.config['QMC']['WormSearchEtaMaxIter']
            once_no_worm = False
            eta_search_failure = False
            for i_try in range(max_iter):
                if self.mpi_rank == 0:
                    print("Eta search: starting iteration {} with eta {}"
                          .format(i_try + 1, eta))
                flush_all_python_output(self.mpi_comm)
                steps_worm, reldiff = eta_count_steps(eta,
                                                      EtaWarmupCounts,
                                                      EtaStepCounts)
                sw = steps_worm.mean()

                # If we have no worm steps after averaging once, the
                # eta will blow up to whatever nan_to_num makes out of
                # inf. If we still have no worm steps after that, we
                # can quite confidently stop the automatic search
                if sw <= 0.0:
                    if once_no_worm:
                        eta_search_failure = True
                        break
                    else:
                        once_no_worm = True

                if approx_equal(sw, steps_worm.stderr(),
                                EtaStepCounts - sw, steps_worm.stderr()):
                    if self.mpi_rank == 0:
                        warn("EtaSearch: step counts are equal within error: "
                             "the relative error might be larger than desirable; "
                             "raise Nwarmups2Plus to reduce it")
                    flush_all_python_output()

                if np.abs(reldiff) > 0.1:
                    eta_stat = (DistributedJackknife(steps_worm)
                                .transform(lambda sw:
                                           eta * (EtaStepCounts - sw) / sw))
                    # no inf or zero
                    eta = np.maximum(np.nan_to_num(eta_stat.mean()),
                                     eta * 1.0 / EtaStepCounts)
                else:
                    if self.mpi_rank == 0:
                        print("Eta search: reached target precision")
                    break
            else:
                eta_search_failure = True
            if eta_search_failure:
                if self.mpi_rank == 0:
                    print('No suitable value of eta found, setting it to 0.')
                eta = 0.0
            if eta_stat is not None and eta == eta_stat.mean():
                eta_err = eta_stat.stderr()
                if self.mpi_rank == 0:
                    print("Final eta: {} +- {}"
                          .format(eta, eta_err))
            ...
            max_iter = self.config['QMC']['WormSearchEtaMaxIter']
            try:
                eta = opt.brentq(lambda eta: eta_count_steps(eta,
                                                             EtaWarmupCounts,
                                                             EtaStepCounts)[1],
                                 bracket_min, bracket_max,
                                 xtol=0.001, rtol=0.001,
                ...
                try:  # for practical purposes, enlarging once should be sufficient.
                    ...
                    eta = opt.brentq(lambda eta: eta_count_steps(eta,
                                                                 EtaWarmupCounts,
                                                                 EtaStepCounts)[1],
                                     0.01 * bracket_min, 100.0 * bracket_max,
                                     xtol=0.001, rtol=0.001,
                                     maxiter=max_iter, disp=True)
                except ValueError:
                    if self.mpi_rank == 0:
    ...
    def solve_worm(self, iter_no, log_function=None):
        ...
        if self.config["QMC"]["WormZMeasG"] != 0:
            def get_g_z_dict(res_z):
                """Return dictionary with possible sources of G measured in Z-space
                that are contained in the provided result argument."""
                gdict = {"gtau": np.average(res_z["gtau"].local, axis=0)}

                if "giw-meas" in res_z:
                    gdict["giw-meas"] = np.average(res_z["giw-meas"].local, axis=0)

                if "gleg" in res_z:
                    gdict["gleg"] = np.average(res_z["gleg"].local, axis=0)

                if "gleg-full" in res_z:
                    gdict["gleg-full"] = np.average(res_z["gleg-full"].local, axis=0)

                return gdict

            giw_z = self.get_giw(self.config,
                                 self.problem,
                                 get_g_z_dict(result_z_other))
            if self.config["QMC"]["offdiag"] == 0:
                giw_z = lattice_convention(giw_z)
            else:
                giw_z = np.ascontiguousarray(giw_z.transpose(4, 0, 1, 2, 3))
            giw_z = DistributedSample([giw_z], self.mpi_comm)
        ...
        giw = DistributedSample([giw], self.mpi_comm)
        if self.config["QMC"]["WormZMeasG"] != 0 and self.config["QMC"]["WormCombineGiw"] != 0:
            res = StatisticalImpurityResult(self.problem, giw, gsigmaiw, smom, self.g_diagonal_only)
            wmethod = self.config["General"]["SelfEnergy"]
            weight_w = np.nan_to_num(1.0/res.calculate_siw(wmethod, giw).stderr()**2,
                                     nan=1.0)
            weight_z = np.nan_to_num(1.0/res.calculate_siw("dyson", giw_z).stderr()**2,
                                     nan=1.0)
            giw = statistics.weighted_combine(giw, giw_z,
                                              weights=(weight_w, weight_z))
        ...
        if self.config["QMC"]["FTGiwGtau"]:
            result_z_other["gtau-ft"] = DistributedJackknife(giw).transform(fourier_transform_giw)
    ...
    def get_resultset(cls, qmc_config):
        ...
        if qmc_config["NLegMax"] > 0:
            result["gleg"] = ctqmc.gleg
            result["gleg-full"] = ctqmc.gleg_full
